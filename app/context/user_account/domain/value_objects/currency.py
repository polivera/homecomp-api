...
class Currency:
    """Value object for currency code (ISO 4217)"""
    ...
    def __post_init__(self):
        if not self._validated:
            ...
            if len(self.value) != 3:
                raise ValueError(f"Currency code must be exactly 3 characters, got {len(self.value)}")
            if not self.value.isalpha():
                raise ValueError(f"Currency code must contain only letters, got {self.value}")
            if not self.value.isupper():
                raise ValueError(f"Currency code must be uppercase, got {self.value}")

    @classmethod
    def from_trusted_source(cls, value: str) -> "Currency":
